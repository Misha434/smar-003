...
RSpec.describe Product, type: :model do
  ...
  it "is valid with a real image data" do
    ...
    @product.image = fixture_file_upload("image/image_test_logo.png")
  ...
  it "is valid with a GIF image data" do
    @product = FactoryBot.build(:product)
    @product.image = fixture_file_upload("image/image_test_3kb.gif")
    expect(@product.valid?).to eq true
  end
  ...
  it "is valid with a JPEG image data" do
    @product = FactoryBot.build(:product)
    @product.image = fixture_file_upload("image/image_test_3kb.jpeg")
    expect(@product.valid?).to eq true
  end
  ...
  it "is valid with a PNG image data" do
    @product = FactoryBot.build(:product)
    @product.image = fixture_file_upload("image/image_test_3kb.png")
    expect(@product.valid?).to eq true
  end
  ...
  it "is invalid with a SVG image data" do
    @product = FactoryBot.build(:product)
    @product.image = fixture_file_upload("image/image_test_3kb.svg")
  ...
  it "is valid with a PSD image data" do
    @product = FactoryBot.build(:product)
    @product.image = fixture_file_upload("image/image_test_3kb.psd")
    expect(@product.valid?).to eq false
  end
  ...
  it "is valid with a BMP image data" do
    @product = FactoryBot.build(:product)
    @product.image = fixture_file_upload("image/image_test_3kb.bmp")
    expect(@product.valid?).to eq false
  end
  ...
  it "is valid with a image data 5MB" do
    @product = FactoryBot.build(:product)
    @product.image = fixture_file_upload("image/image_test_5mb.jpeg")
    expect(@product.valid?).to eq true
  end
